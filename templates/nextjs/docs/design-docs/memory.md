...
1. Reads `.rigel/gate-failures.jsonl` — every gate FAIL this project has recorded, each with a
   **signature** `{gate}:{discriminator}` (e.g. `eslint:no-restricted-imports`, `tsc:TS2345`,
   `arch:layers-test-missing`). Signatures are stable across files, so the same class of
...
2. Groups failures by signature and matches each against existing lessons' `signatures`:
   ...
   - A *coarse* signature (e.g. a bare `tsc:TS2345`) that maps to more than one candidate lesson
...
1. Writes the actual check — an ESLint rule, a `grep` in `.claude/hooks/post-write.sh`, or a gate
...
```bash
node scripts/verify-promotion.mjs LSN-XXXX                     # checks the rule doesn't break the build
node scripts/verify-promotion.mjs LSN-XXXX --tamper-verified   # after proving it catches the failure
```
