...
function runTest(path) {
  const r = spawnSync('npx', ['vitest', 'run', '--reporter=json', path], { encoding: 'utf8' })
  const raw = (r.stdout || '').slice((r.stdout || '').indexOf('{'))
  let j
  try {
    j = JSON.parse(raw)
  } catch {
    die(`vitest produced no readable report for ${path} (exit ${r.status}) — a TOOL failure, not a test result\n      ${(r.stderr || '').trim().split('\n').slice(-2).join(' ')}`)
  }
  // A suite that fails to LOAD reports zero assertions — vitest surfaces it the same way jest does.
  const execError = (j.testResults ?? []).find((t) => t.status === 'failed' && !t.assertionResults?.length)
  if (execError) {
    die(
