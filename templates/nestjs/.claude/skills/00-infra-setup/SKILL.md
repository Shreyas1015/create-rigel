...
"gate": "npm run verify:rigel && npm run typecheck && npm run lint && npm run test:arch && npm run mcp:check && npm run knowledge && npm run debug:regression check && npm run contract:gate",
