...
1. **A declared server that cannot start is a silent no-op.** It loads as "configured", provides no
   ...
   as a test runner that executes zero tests. `node scripts/check-mcp.mjs` is a gate step precisely so that
...
```bash
node scripts/check-mcp.mjs      # or `make gate`, which runs it as one step
```

> This template has no `package.json`; the gate lives in `scripts/gate.sh`. The checker is still
> JavaScript because `.mcp.json` is Claude Code's file and Claude Code is a Node program — there is
...
node scripts/check-mcp.mjs                       # then prove it can actually start
```
...
assuming your setup is broken — and the MCP check will tell you which entry is at fault rather
