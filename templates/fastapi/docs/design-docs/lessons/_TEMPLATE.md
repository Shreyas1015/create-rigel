...
first_seen: PLAN-XXX
last_seen: PLAN-XXX
signatures: [] # gate-failure signatures that map here — e.g. ["ruff:F401", "arch:isolation-test-missing"]. /curate matches on these.
enforced_by: null # once ENFORCED: the mechanical check, e.g. "ruff:TID252" or "post-write.sh:httpexception-in-service". The prose below is then deleted.
---
