...
2. a single one-line entry under a skill's `## Known failure modes` heading — **only** for a lesson
   that is already `DISTILLED` or `ENFORCED` (Step 6).
...
person writes the ruff rule / `post-write.sh` grep / gate step, sets `status: ENFORCED`, and
...
```markdown
## Known failure modes
- LSN-0031: Every repo return is Schema.model_validate(...) — never a raw row.__dict__.
```
...
Curated N failure signature(s):
  ...
  ↳ skills updated:         [05-build-layer ← LSN-…]
```
