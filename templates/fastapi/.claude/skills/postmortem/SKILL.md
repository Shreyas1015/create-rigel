...
<REQUIRED. Be specific enough to build: a ruff rule on X, a grep in post-write.sh for Y,
a step in scripts/gate.sh asserting Z, an arch test under tests/architecture/ that fails when W.
"More testing" is not an answer.>
```
...
Next: build that check, then
  python3 scripts/verify_promotion.py LSN-XXXX --tamper-verified
