...
export function resolveAnchor(root, anchor, srcIndex) {
  ...
  if (anchor.symbol) {
    const re = new RegExp(
      `\\b(class|interface|type|enum|const|function|def|struct|model)\\s+${escapeRe(anchor.symbol)}\\b`,
    )
 ...
 */
export function checkKnowledge(root, knowledgeDir = KNOWLEDGE_DIR) {
  const domain = join(root, knowledgeDir, 'domain')
  const result = { checked: 0, entries: [], problems: [] }
