...
// ── build + read ────────────────────────────────────────────────────────────────
/**
 * @param owned     PLAN-013: the paths Rigel actually wrote, when the caller knows them (adoption
 *                  does). Omit it and we fall back to hashing every managed path on disk, which is
 *                  correct ONLY when Rigel wrote the whole tree — i.e. a greenfield scaffold.
 * @param baseline  paths that already existed and were declined. Never owned, never enforced;
 *                  recorded so `update` and `doctor` can tell "not ours" from "not shipped".
 * @param mode      'greenfield' | 'brownfield' — how this repo met Rigel. Informational; every
 *                  behavioural decision reads `files`/`baseline`, never this.
 */
export function buildManifest({
  root, template, version, source, layer = null, answers = {}, ownership, now,
  owned = null, baseline = [], mode = 'greenfield', adoptedAt = null,
}) {
  const files = owned
    ? Object.fromEntries(
        owned
          .filter((p) => classify(p, ownership) === 'managed')
          .sort()
          .map((p) => [p, hashFile(join(root, p))]),
      )
    : hashManaged(root, ownership)
  return {
