...
// ── the enforced-layer boundary (PLAN-014) ──
// This list is the ONLY definition; doctor imports it. Two copies is how a check and its report
// end up reporting different numbers — a drift this repo has already been bitten by twice.
{
  assert.ok(isEnforced('src/services/order.service.ts'))
  assert.ok(isEnforced('src/repo/order.repo.ts'))
  assert.ok(!isEnforced('src/controllers/orders.js'), 'a legacy layout is simply not governed')
  assert.ok(!isEnforced('src/lib/helpers.ts'))
  assert.ok(!isEnforced('src.ts'), 'a prefix match must not leak: "src/types" ≠ "src/typesomething"')
  assert.ok(!isEnforced('src/typescript-utils/x.ts'), 'the separator is required')

  // Only source files are migration candidates, and the result is stable order.
  assert.deepEqual(
    unenforced(['src/controllers/b.ts', 'README.md', 'src/services/x.ts', 'src/controllers/a.ts']),
    ['src/controllers/a.ts', 'src/controllers/b.ts'],
  )
  assert.deepEqual(unenforced([]), [])
  assert.ok(ENFORCED_LAYERS.every((l) => l.startsWith('src/')))
}
