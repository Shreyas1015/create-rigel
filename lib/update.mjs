...
export const STAMPED = [
  ...
  // PLAN-023: the design gate's two halves — trigger derivation, and citation resolution.
  {
    from: (H) => join(H, 'lib', 'decisions.mjs'),
    to: () => join('scripts', 'lib', 'rigel-decisions.mjs'),
  },
  {
    from: (H) => join(H, 'lib', 'design.mjs'),
    to: () => join('scripts', 'lib', 'rigel-design.mjs'),
  },
]
