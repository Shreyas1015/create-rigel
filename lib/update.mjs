...
export const STAMPED = [
  ...
  // PLAN-009: the knowledge checker's library. JS everywhere for now — the anchor resolver is
  // language-agnostic (it greps source text), so fastapi can run it via node if present; its gate
  // treats knowledge as advisory, so a missing node is not a failure there.
  {
    from: (H) => join(H, 'lib', 'knowledge.mjs'),
    to: () => join('scripts', 'lib', 'rigel-knowledge-lib.mjs'),
  },
]
