...
Deleted model — this anchor is dead.`)
  ...
  assert.deepEqual(checkKnowledge(root), { checked: 0, skippedNotOwner: 0, entries: [], problems: [] })
  rmSync(root, { recursive: true, force: true })
}

// ── owner gating: the whole glossary reaches every service, but only its OWNER verifies it ──
// Without this, flipping the check to blocking red-lights every consumer for terms it merely reads.
{
  const root = mkdtempSync(join(tmpdir(), 'kn-own-'))
  const w = (p, body) => {
    mkdirSync(join(root, dirname(p)), { recursive: true })
    writeFileSync(join(root, p), body)
  }
...
anchors:
  - symbol: Shipment
---
Owned by billing; every service reads it.`)

  // a consumer that owns none of that code: read, don't verify
  const asConsumer = checkKnowledge(root, 'knowledge', { service: 'acme-web' })
  assert.equal(asConsumer.problems.length, 0, 'a consumer must not fail on a term it does not own')
  assert.equal(asConsumer.skippedNotOwner, 1, 'and the skip is reported, not silent')

  // the OWNER must still be held to it
  const asOwner = checkKnowledge(root, 'knowledge', { service: 'acme-billing' })
  assert.equal(asOwner.problems.length, 1, 'the owner is still accountable for its own anchors')
  assert.match(asOwner.problems[0], /symbol not defined/)

  // no service given (single-repo project) → check everything, no ceremony required
  assert.equal(checkKnowledge(root, 'knowledge').problems.length, 1)
  rmSync(root, { recursive: true, force: true })
}

// ── widened definition regex: a style we don't recognise would be a false FAILURE once blocking ──
{
  const root = mkdtempSync(join(tmpdir(), 'kn-sym-'))
  const w = (p, body) => {
    mkdirSync(join(root, dirname(p)), { recursive: true })
    writeFileSync(join(root, p), body)
  }
  w('src/a.ts', 'export let Alpha = 1\nexport var Beta = 2\nexport function* Gamma() {}\n')
  const idx = buildSourceIndex(root)
  for (const sym of ['Alpha', 'Beta', 'Gamma']) {
    assert.equal(resolveAnchor(root, { symbol: sym }, idx).ok, true, `${sym} should resolve`)
  }
  rmSync(root, { recursive: true, force: true })
}
