...
import { sourceFiles } from './impact.mjs'
...
/** Directories whose contents Rigel's layer rules and coverage thresholds actually govern. */
const ENFORCED_LAYERS = [
...
function convergence(root, manifest) {
  ...
  const src = sourceFiles(root, ['src'])
  if (src.length) {
    const outside = src.filter((p) => !ENFORCED_LAYERS.some((l) => p.startsWith(l + '/')))
