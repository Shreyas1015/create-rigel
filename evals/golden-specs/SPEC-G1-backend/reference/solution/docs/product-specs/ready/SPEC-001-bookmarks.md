...
> Working id note: the create-rigel eval harness resolves spec ids via `/SPEC-\d+/` from the
> active plan, so this golden spec is filed under `SPEC-001` in this reference build. The 4
> acceptance criteria and their tests keep their canonical AC-1..AC-4 ids and grade against
> SPEC-G1's 4 ACs unchanged.
